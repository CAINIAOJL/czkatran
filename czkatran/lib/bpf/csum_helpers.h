...
*/
__always_inline static __u16 csum_fold_helper(__u64 csum) {
  int i;
#pragma unroll //循环展开
  for(i = 0; i < 4; i++) {
    if(csum >> 16) {
...
ipv6_csum(void* data_start, int data_size, __u64* csum, struct ipv6hdr* ip6h) {
  // ipv6 pseudo header
  __u32 tmp = 0;
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&ip6h->saddr), sizeof(struct in6_addr), *csum);
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&ip6h->daddr), sizeof(struct in6_addr), *csum);
  tmp = __builtin_bswap32((__u32)(data_size));
  *csum = bpf_csum_diff(0, 0, &tmp, sizeof(__u32), *csum);
...
//地址和长度都需以网络字节序（大端序）提供，
//但bpf_csum_diff期望的是主机字节序，
//所以使用__builtin_bswap32函数在传递之前转换32位整数的字节序。
__always_inline static void ipv6_csum(
  void* data_start,
  int data_size,
  __u64* csum,
  ...
  __u32 tmp = 0;
  //ip地址是32位=__be32
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&ipv6_hdr->saddr), sizeof(struct in6_addr), *csum);
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&ipv6_hdr->daddr), sizeof(struct in6_addr), *csum);
  tmp = __builtin_bswap32((__u32)(data_size));
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&tmp), sizeof(__u32), *csum);
  tmp = __builtin_bswap32((__u32)(ipv6_hdr->nexthdr));
  *csum = bpf_csum_diff(0, 0, (unsigned int*)(&tmp), sizeof(__u32), *csum);
